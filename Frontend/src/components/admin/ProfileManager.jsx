    ...
    <section className="w-full">
      ...
      <div className="space-y-6">
        ...
        >
          <div className="grid gap-5 md:grid-cols-2">
            {/*
              FIX: "Resume URL" was previously a free-text editable
              field bound to `profile.resume.url`. Since that field
              is auto-generated by the server on every upload, admins
              editing/saving THIS page (even for unrelated fields
              like About or Skills) were silently overwriting the
              correct backend-generated URL with a stale value.

              It is now a read-only display so it can never be
              accidentally overwritten. Manage your resume from the
              "Resume" tab in the dashboard instead.
            */}

            <div>
              <span className={labelClass}>
                Resume URL (auto-managed)
              </span>

              <div className="mt-2 rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-3 text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
                {profile.resume.url || 'Not set yet'}
              </div>

              <p className="mt-1.5 text-xs text-gray-400">
                Upload/replace the resume from the Resume tab — this updates automatically.
              </p>
            </div>

            {/*
              FIX: fileName is the Cloudinary public_id — also
              server-authoritative, same reasoning as the URL field
              above. Read-only now.
            */}

            <div>
              <span className={labelClass}>
                File Name (auto-managed)
              </span>

              <div className="mt-2 rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-3 text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
                {profile.resume.fileName || 'Not set yet'}
              </div>
            </div>
